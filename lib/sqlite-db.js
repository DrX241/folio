...
function cleanSQL(sql) {
  ...
  debugLog('SQL brut reçu:', sql.substring(0, 200));
  ...
  debugLog('SQL nettoyé:', cleaned.substring(0, 200));
...
export function executeSelectQuery(sql) {
  ...
  try {
    // Log pour débogage
    debugLog('Exécution SQL:', cleanedSQL.substring(0, 200));
    
    const stmt = db.prepare(cleanedSQL);
    const results = stmt.all();
    return results;
  } catch (error) {
    debugError('Erreur SQL complète:', {
